...
from roi_data_layer.ohem_minibatch import get_allrois_minibatch, get_ohem_minibatch
import numpy as np
import yaml
...
class OHEMDataLayer(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        cls_prob = bottom[0].data # cls_prob_readonly
        bbox_pred = bottom[1].data # bbox_pred_readonly
        rois = bottom[2].data # rois
        labels = bottom[3].data # labels
        if cfg.TRAIN.BBOX_REG:
            bbox_target = bottom[4].data # bbox_targets
            bbox_inside_weights = bottom[5].data # bbox_inside_weights
            bbox_outside_weights = bottom[6].data # bbox_outside_weights
        else:
            bbox_target = None
            bbox_inside_weights = None
            bbox_outside_weights = None

        flt_min = np.finfo(float).eps # min float
        # classification loss
